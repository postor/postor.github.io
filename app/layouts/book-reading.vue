...
<template>
  <div class="flex flex-col min-h-screen">
    <header class="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 h-14 border-b border-slate-200 bg-white">
      <div class="w-10"></div>
    ...
    <nav class="fixed bottom-0 left-0 right-0 z-50 flex flex-row justify-evenly border-t border-slate-200 bg-white h-14">
      <button
        :class="[
          'flex flex-col items-center justify-center flex-1 h-full px-2 py-1 text-sm font-medium transition-colors',
          tab==='recent' ? 'text-blue-600 font-bold border-b-2 border-blue-600' : 'text-slate-700',
      ...
      <button
        :class="[
          'flex flex-col items-center justify-center flex-1 h-full px-2 py-1 text-sm font-medium transition-colors',
          tab==='files' ? 'text-blue-600 font-bold border-b-2 border-blue-600' : 'text-slate-700',
          'disabled:text-slate-400 disabled:cursor-not-allowed'
        ]"
        @click="goTab('files')"
        :disabled="false"
      ...
      <button
        :class="[
          'flex flex-col items-center justify-center flex-1 h-full px-2 py-1 text-sm font-medium transition-colors',
          tab==='read' ? 'text-blue-600 font-bold border-b-2 border-blue-600' : 'text-slate-700',
          'disabled:text-slate-400 disabled:cursor-not-allowed'
        ]"
        @click="goTab('read')"
        :disabled="!canRead"

  ...
  try {
    ...
    if (!catPath) {
        ...
        if (cat.categories) {
          for (const sub of cat.categories) {
            items.push({ 
              type: 'category', 
              title: sub.title, 
              href: `/learn-game?category=${encodeURIComponent(cat.title + '/' + sub.title)}` 
            })
          }
        }
      ...
      for (let i = 0; i < res.breadcrumbs.length; i++) {
        ...
        const href = `/learn-game/?category=${encodeURIComponent(res.breadcrumbs.slice(0, i + 1).map((x: any) => x.title).join('/'))}`
...
function buildDisplay(children: any, res?: any) {
  ...
  for (const c of children.categories || []) {
    ...
    items.push({ type: 'category', title: c.title, href: `/learn-game/?category=${encodeURIComponent(fullPath)}` })
  }
  // then games
  for (const g of children.games || []) {
    items.push({ type: 'game', title: g.title, href: `/learn-game/game?cfg=${encodeURIComponent(g.path)}` })
  }
